...
- Axios is wrapped by `client/src/composables/useHttp.ts`.
...
- API access should go through `useHttp()` unless there is a concrete reason to use raw Axios.
...
- SCSS token variables are forwarded from `client/src/styles/core/_tokens.scss` through `client/src/styles/resources.scss`.
